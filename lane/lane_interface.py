...
from lane import visualize_car_detection
from lane import process_image as ip
from lane import model
...
def executemodel(imagename):
    img_arr    = cv2.imread('C:/Users/Esoundarajan/Desktop/Elango/Personal/DBS/Adithya_Chowdary/Lane_Detection_Code/input/' + imagename)
    config = InferenceConfig()

    NN = model.MaskRCNN(mode="inference", model_dir='logs', config=config)
    NN.load_weights('./lane/mask_rcnn_coco.h5', by_name=True)
    img_result = process_video(NN,img_arr)
    cv2.imwrite('C:/Users/Esoundarajan/Desktop/Elango/Personal/DBS/Adithya_Chowdary/Lane_Detection_Code/static/images/' + 'result_' + imagename, img_result)

    img_result = findLane(img_arr)
    cv2.imwrite('C:/Users/Esoundarajan/Desktop/Elango/Personal/DBS/Adithya_Chowdary/Lane_Detection_Code/static/images/' + 'result_obj_' + imagename, img_result)
    results = {
        "imagePath": 'result_' + imagename,
        "obj_result_imagePath": 'result_obj_' + imagename
    };
